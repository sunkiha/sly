...
public class MainActivity extends BaseActivity implements View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		imUser.setId("123456");
		imUser.setAvatar("12.jpg");
		imUser.setNickname("sssd");
		UserManager.saveUserInfo(imUser);
		MobSDK.setUser(imUser.getId(), imUser.getNickname(), imUser.getAvatar(), null);
       Toast.makeText(this,UserManager.getUser().getId(),Toast.LENGTH_LONG).show();
		/*if (UserManager.getUser() == null) {
			login();
		} else {
			showTabConversation();
			System.out.println("sly"+UserManager.getUser().getId());
		}*/
	//	System.out.println("sly"+UserManager.getUser().getId());
		checkPermissions();
	}
	...
	private void login() {
		...
		//如果用户未登录，则登录
	//	MobSDK.clearUser();
/*		UserManager.logout(new MobIMCallback<Boolean>() {
			@Override
...
/*		UserManager.login(new MobIMCallback<IMUser>() {
			public void onSuccess(IMUser imUser) {
				...
				//MobSDK.setUser(imUser.getId(), imUser.getNickname(), imUser.getAvatar(), null);
				MobSDK.setUser("12344444", "用户昵称","12.jpg", null);
				System.out.println("sly"+UserManager.getUser().getId());
				showTabConversation();
			}
