            ...
            if (this.allowDrag && panel.allowDrag && mini.isAncestor(panel.getHeaderEl(), e.target) && !mini.findParent(e.target, "mini-tools")) {
                ...
                var drag = new mini.Drag({
                    ...
                    onStop: function () {
                        ...
                        //记录当前portal块图表顺序
                        var columnIds="";  
                        var columnTemp="";//临时存储当前元素上边节点,方便再倒序
                        var previous=sf._dragProxy.previousSibling;//获取当前对象前边对象
                        while(previous){//存在前一个元素,获取元素标识集合
                        	var targetPanel = mini.get(previous);                        	
                        	columnTemp+=targetPanel.id+",";
                        	previous=previous.previousSibling;
                        }
                        //将数据倒序转变为正序
                        var tempArr=columnTemp.split(",");
                        for(var i=(tempArr.length-1);i>=0;i--){
                        	columnIds+=tempArr[i]+",";
                        }
                        while(next){//存在下一个元素,获取元素标识集合
                        	var targetPanel = mini.get(next);                        	
                        	columnIds+=targetPanel.id+",";
                        	next=next.nextSibling;
                        }                       

                        jQuery(sf._dragProxy).remove();
                        sf._maskProxy = null;
                        jQuery("td > div[class='mini-portal-proxy']").remove();
                        panel.el.style.position = "static";
                        panel.setWidth("100%");
                        mini.setOpacity(panel.el, 1);
                        if(sf.ajaxUrl!=null && sf.ajaxUrl!=""){//存在异步路径时执行异步方法,并在方法后补充两个参数,当前移动的portal标识id及列索引colIndex  
                        	var url=sf.ajaxUrl;
                        	if(url.indexOf("?")==-1){
                        		url+="?id="+t.id+"&colIndex="+column+"&columnIds="+columnIds;
